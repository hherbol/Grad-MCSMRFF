...
def bfgs(run_name, step_size=0.05, step_size_adjustment=0.5, maxiter=1000,
    ...
    def rebuild_params(params, N, N_tersoff):
        parameters[0] = params[:N].reshape((-1, N_tersoff))
    ...
    def apply_restrictions(params, p_hold, dr, nLJ):
        ...
        # Recall, dr is the change in error. Thus a negative one implies the
        # new error is smaller than the old one
    ...
    atoms, systems_by_composition = mcsmrff_utils.get_training_set(
        run_name, use_pickle=True, pickle_file_name=training_set_file_path
    )

    print("\n\nStep        Avg Energy        Avg rms_force        \
error_force (%)        error_energy (%)\
             \n----------------------------------------------------\
--------------------------------------")
    sys.stdout.flush()
    ...
    N_tersoff = len(current_parameters[0][0])
    ...
    mcsmrff_gradient.run_lammps(atoms, systems_by_composition, tersoff_atoms,
                                current_parameters[0], current_parameters[1],
                                current_parameters[2], run_name)
    energies, rms_forces = mcsmrff_files.parse_lammps_output(run_name,
                                                             len(atoms.atoms))
    ...
    if linesearch is 'armijo':
        if display > 1:
            print("armijo linesearch "),

        def check_backtrack(f1, f0, gk, pk,
                            armijio_line_search_factor, step_size):
            chk = armijio_line_search_factor *\
                step_size *\
                np.dot(gk, pk)
            return f1 - f0 > chk
    else:
        if display > 1:
            print("default linesearch "),
    ...
    while (loop_counter < maxiter):
        ...
        # Here we drop ljparams and tersoff_params into a 1D array and get
        # the gradient
        working_parameters = np.append(
        ...
        error_force, error_energy = mcsmrff_gradient.calculate_error(
            run_name,
            len(atoms.atoms))
        error_force *= 100.0
        error_energy *= 100.0
...
%.2f" % (loop_counter,
        ...
        # Prevent division by 0 in situations that we have 0/0 because
        ...
        if max(step_lengths) < MIN_STEP:
            if display > 0:
                print("Error - Step size unreasonable (%lg)"
                      % abs(max(step_lengths))),
            break

        # If we have too large of a step size, set to max
        # TODO: Maybe scale everything so we remain on the eigenvector
        indices_of_large_steps = [(i, s) for i, s in enumerate(step_lengths)
                                  if s > max_step_size]
        for i, s in indices_of_large_steps:
            step_direction[i] *= max_step_size / s
        ...
        # Hold new parameters
        # We do so because we may not want to keep them if it makes the
        # next step bad.
        new_working_parameters = working_parameters +\
            step_size * step_direction

        new_parameters = rebuild_params(new_working_parameters,
                                        nLJ,
                                        N_tersoff)

        new_parameters = apply_restrictions(new_parameters,
                                            current_parameters[2],
                                            step_direction,
                                            nLJ)

        # Get the new gradient and check if max has increased
        mcsmrff_gradient.run_lammps(atoms,
        ...
        new_energies, new_rms_forces = mcsmrff_files.parse_lammps_output(
            run_name,
            len(atoms.atoms))
        ...
        # This allows for the edge case in which after decreasing step_size, a
        # situation arises in which larger alphas are acceptable again. Thus,
        # we reset to the original step_size
        else:
            reset_step_size -= 1
            ...
            if reset_step_size < 0 and step_size < ALPHA_CONST:
                ...
                step_size = ALPHA_CONST
                step_size_adjustment = BETA_CONST
                reset_step_size = RESET_CONST
        ...
        A1 = I - \
            change_in_parameters[:, np.newaxis] *\
            change_in_gradient[np.newaxis, :] * rhok

        A2 = I - \
            change_in_gradient[:, np.newaxis] * \
            change_in_parameters[np.newaxis, :] * rhok

        current_Hessian = np.dot(A1,
                                 np.dot(current_Hessian,
                                        A2)) +\
                                (rhok * change_in_parameters[:, np.newaxis] *
                                 change_in_parameters[np.newaxis, :])
        ...
        if not os.path.isdir("parameters"):
            os.mkdir("parameters")
        if not os.path.isdir("parameters/%s" % run_name):
            os.mkdir("parameters/%s" % run_name)
        os.chdir("parameters/%s" % run_name)
        mcsmrff_files.write_params(parameters[0],
                                   parameters[1],
                                   parameters[2],
                                   run_name,
                                   append="_%d" % loop_counter)
        os.chdir("../../")
        ...
        energies, rms_forces = mcsmrff_files.parse_lammps_output(
            run_name,
            len(atoms.atoms))
