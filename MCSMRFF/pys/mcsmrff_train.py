...
def generate_training_set(N_perterbations_per_seed=5,
    ...
    if os.path.exists("./%s" % training_sets_folder):
        raise Exception("Warning - Would overwrite training set folder \
already in existance.")
    os.mkdir(training_sets_folder)
    ...
    for mol in seeds_B:
        files.write_cml(mol, name="%s/%d" % (training_sets_folder,
    ...
    for mol in seeds_B:
        for i in range(N_perterbations_per_seed):
            ...
            files.write_cml(m, name="%s/%d" % (training_sets_folder,
                                               training_set_counter))
            training_set_counter += 1
    ...
    for mols in seeds_A:
        ...
        for m1, m2 in to_run:
            axis = np.array(m2.get_center_of_mass()) -\
                np.array(m1.get_center_of_mass())
            ...
            for i in range(N_expansion):
                ...
                for j in range(N_perterbations_per_expansion):
                    m3 = copy.deepcopy(m1)
                    m4 = copy.deepcopy(m2)
                    m3.perterbate(dx=expansion_perterbation_dx,
                                  dr=expansion_perterbation_dr)
                    m4.perterbate(dx=expansion_perterbation_dx,
                                  dr=expansion_perterbation_dr)
                    files.write_cml([m3, m4], name="%s/%d"
...
def run_high_level(training_sets_folder="training_set", procs=1, queue=None):
    ...
    for i in frange:
        ...
        if prev_converged:
            running_jobs.append(orca.job("ts_%d_high" % i, route,
                                         atoms=[],
                                         extra_section=extra_section,
                                         charge=charge,
                                         grad=True,
...
# This function will read in all orca simulations from a folder
# "training_set" in the current working directory. If use_pickle is True,
# then it will first check to see if a pickle file exists in the
# training_sets folder under the name "training_set.pickle". If file does
# not exist - Read in all data from subfolders and generate the file. If file
# does exist - Read in the pickle file if it exists. If pickle_file_name is
# a string, then it will read in the pickle file specified by the string path
...
def create_training_set(run_name,
    ...
    generate_training_set(
        ...
        perterbation_dr=perterbation_dr,
        expansion_perterbation_dx=expansion_perterbation_dx,
        expansion_perterbation_dr=expansion_perterbation_dr,
    ...
    for j in jobs2:
        ...
        shutil.copyfile("%s/%d.cml" % (training_sets_folder, i),
    ...
    for fptr in os.listdir("orca"):
        if not os.path.isdir("orca/" + fptr):
            continue
...
if __name__ == "__main__":
    create_training_set("set1",
                        N_perterbations_per_seed=1,
                        perterbation_dx=0.3,
                        perterbation_dr=10,
                        expansion_perterbation_dx=0.1,
                        expansion_perterbation_dr=0.5,
                        expansion_step=0.5,
                        N_expansion=1,
                        N_perterbations_per_expansion=1,
                        min_seeds=False,
                        training_sets_folder="training_set",
                        queue=None,
                        procs=1,
