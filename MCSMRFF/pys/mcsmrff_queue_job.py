...
def job(run_name, parameters, path="", new_pdf_props={}, new_opt_props={}, disregard=[]):
	pdf_props={"start":0.0, "stop":5.0, "step":0.01, "cutoff":10.0, "persist":"False", "quanta":0.001}
	opt_props={"step_size":0.1, "maxiter":1000, "perturbation":1.01, "training_set_file_path":"/fs/home/hch54/MCSMRFF/Grad-MCSMRFF/MCSMRFF/training_sets/training_set.pickle"}
	...
	for prop in new_opt_props:
		if prop in opt_props:
			...
			raise Exception("The optimizer parameter %s does not exist." % prop)
...
new_parameters = BFGS("$RUN_NAME$", step_size=$STEP_SIZE$, maxiter=$MAXITER$, perturbation=$PERTURBATION$, param_file="$PARAMETERS$", three_body=perturbate_these, tersoff=None, lj_coul=None, opt="Force", reset_step_size=5, training_set_file_path=\"$TRAINING_SET_FILE_PATH$\")
...
'''
	...
	for prop in opt_props:
		file_string = file_string.replace("$%s$" % prop.upper(), str(opt_props[prop]))
