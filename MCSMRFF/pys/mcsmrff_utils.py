...
def read_dump_worker(fptr, data=["element", "x", "y", "z"]):
    ...
    for line in open(fptr, 'r'):
        if line.startswith("ITEM: ATOMS"):
            ...
            if ids is None:
                ids = {name: index
                       for index, name in enumerate(line.split()[2:])}
                continue
...
# "training_sets" in the current working directory. If use_pickle is True,
# then it will first check to see if a pickle file exists in the
# training_sets folder under the name "training_set.pickle".
# If file does not exist - Read in all data from subfolders
# and generate the file.
# If file does exist - Read in the pickle file if it exists
# If pickle_file_name is a string, then it will read in the pickle file
