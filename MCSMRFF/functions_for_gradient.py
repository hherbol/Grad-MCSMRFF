...
def parse_tersoff_file(filename):
	################################################################################################################################################
	## There was an error in using re.findall when reading in our newly made param files. So this reads it in instead (I dunno how to regex :'( ) ##
	################################################################################################################################################
...
def read_params(run_name, exact=False): #read in a file and output a list of all the TERSOFF parameters for a file of the format shown below in the order shown below
...
def get_gradient(parameters, system, systems_by_composition, run_name, perturbation=1.01, three_body=None):
	...
	for i,p in enumerate(p_tersoff):
		...
		#if chk != prv or prv is None:
			#prv = chk
			#print chk,

		#print("Running on %d" % int(i/14) )
...
def steepest_descent(run_name, alpha=0.05, maxiter=1000, gtol=1E-3, perturbation=1.01, param_file=None, three_body=None): #better, but tends to push error up eventually, especially towards endpoints.
	...
	#print("\nTERSOFF STRINGS:\n")
	#print np.array(parameters[1]).reshape((-1,3))
	...
	while (rms_gradient > gtol) and (step < maxiter):
		...
		# Force no change on parameters we do not want to change
		index = indices_of_desired_three_body(parameters[1], three_body)
	...
	print("\n\nSAVING NEW PARAMETERS TO %s_o.tersoff\n\n" % run_name)
...
if __name__ == "__main__":
	...
	run_mcsmrff("test_start", test_system, old_parameters, RUN="20000")
	...
	new_parameters = steepest_descent("test", alpha=0.2, maxiter=100, perturbation=1.01, param_file="reasonable.tersoff", three_body=perturbate_these)
	
	# See how the updates did
	run_mcsmrff("test_end", test_system, new_parameters, RUN="20000")
